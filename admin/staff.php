...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['requestType']) && $_POST['requestType'] === 'addStaff') {
        ...
        $save = $connect->prepare("INSERT INTO staff(username,password,name,surname) VALUES (:username,:password,:name,:surname)");
    ...
    if (isset($_POST['requestType']) && $_POST['requestType'] === 'deleteStaff') {
            ...
            $delete = $connect->prepare("DELETE FROM staff WHERE username=:username");
    ...
    if (isset($_POST['requestType']) && $_POST['requestType'] === 'getStaff') {
        $username = $_POST['username'];
        $query = $connect->prepare("SELECT name,surname,username FROM staff WHERE username = :username");
    ...
    if (isset($_POST['requestType']) && $_POST['requestType'] === 'changeStaff') {
        ...
        if ($oldUsername) {
            if ($oldUsername !== $_POST['staffusername']) {
                $query = $connect->prepare("SELECT username FROM staff WHERE username = :username");
                ...
                if ($row) {
                    echo "Aynı kullanıcı adına sahip birden fazla personel olamaz!";
                    exit;
                }
            }
            ...
            <div class="col col-md">
                ...
                <table class="center flx">
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT name,surname,username FROM staff ORDER BY name LIMIT $startFrom, $itemsPerPage";
                        ...
                        $totalStmt = $connect->query("SELECT COUNT(*) as total FROM staff");
