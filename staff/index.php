...
if (empty($_SESSION['swiftlystaff'])) {
...
if (isset($_POST['requestType']) && $_POST['requestType'] === 'loginStaff') {
  $usern = $_POST["username"];
  $passw = hash('sha256', $_POST["password"]);
  $sql = "SELECT id,username,password FROM staff WHERE username=:username AND password=:password";
  ...
  if ($query->rowCount() > 0) {
    ...
    $auth = openssl_encrypt("personel", $cpr, $enckey, $ops, $enc_iv);
    ...
    $_SESSION['swiftlystaff'] = sha1(md5("nmHN738NDSSasdaU26d62Naj"));
    echo "localStorage.setItem('username','$usern'); document.location = 'main.php';";
  }else{
  ...
  <main>
    ...
    <svg class="backb hover" width="20" height="20" onclick="window.location.href='../index.php'">
      ...
      <div class="hmid formh">
        ...
        <h2 class="fll">Personel</h2>
      </div>
      <form id="loginStaff" onsubmit="return posty(event,'index.php','login','response')">
